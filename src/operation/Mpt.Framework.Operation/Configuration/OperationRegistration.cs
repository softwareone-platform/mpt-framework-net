...
internal interface IOperationRegistration
{
    OperationDescriptor Descriptor { get; }

    abstract void RegisterStateMachine(IBusRegistrationConfigurator<IOperationBus> busConfigurator, OperationSettings settings, IOperationPersistenceProvider persistence);
...
internal class OperationRegistration<TOperation, TTask, TSaga>(OperationDescriptor descriptor) : IOperationRegistration
    ...
    where TTask : class
{
    ...
    public void RegisterStateMachine(IBusRegistrationConfigurator<IOperationBus> busConfigurator, OperationSettings settings, IOperationPersistenceProvider persistence)
    {
    ...
    public void RegisterEndpoints<T>(IBusRegistrationContext context, IBusFactoryConfigurator<T> busConfigurator, OperationSettings settings)
        where T : IReceiveEndpointConfigurator
    {
        if (settings.Mode == OperationMode.Dispatch)
            return;
