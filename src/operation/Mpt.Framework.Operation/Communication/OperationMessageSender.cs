    ...
    where TOperation : IOperationContract
{
    ...
    public OperationMessageSender(IOperationBus bus, OperationSettings settings, IOperationProvider operationProvider)
    {
        _bus = bus;
        ...
        {
            OperationTransport.InMemory => "queue",
            OperationTransport.ServiceBus => "topic",
    ...
    private async Task<ISendEndpoint> GetEndpontAsync(MessageGroup messageGroup, CancellationToken cancellationToken)
    {
        ...
        {
            OperationTransport.InMemory => _descriptor.GetQueueName(messageGroup),
            OperationTransport.ServiceBus => _descriptor.TopicName,
            _ => throw new InvalidOperationException($"Unsupported transport type {_settings.Transport}"),
        };
