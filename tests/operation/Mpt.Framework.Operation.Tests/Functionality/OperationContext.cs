...
namespace Mpt.Framework.Operation.Tests.Functionality;

/// <summary>
/// Spins up a self-contained in-memory operation engine in DI, dispatches a single operation of the
    ...
    where TOperation : IOperationContract, new()
{
    ...
    {
        var provider = MakeOperationProvider();
        var scope = provider.CreateScope();
        _dispatcher = scope.ServiceProvider.GetRequiredService<IOperationDispatcher>();
        _busControl = (provider.GetRequiredService<IOperationBus>() as IBusControl)!;
    ...
    private ServiceProvider MakeOperationProvider()
    {
        ...
        services.AddLogging();
        services.AddOperation("test", ops =>
        {
            ops.Settings.Mode = OperationMode.ConsumeAndDispatch;
            ops.Settings.Transport = OperationTransport.InMemory;
