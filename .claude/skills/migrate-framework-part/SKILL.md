...
| `Mpt.Framework.Application.Rules.EntityAction` and `Mpt.Framework.Application.Rules.EntityEventTypes` enums (also referenced from the new `IEntityEventProducer<T>`) | **Duplicate**, don't share. Both already exist in `Mpt.Framework.Persistence.Abstractions` (namespace `Mpt.Framework.Persistence`); the MessageHub.Events migration adds parallel copies in `Mpt.Framework.MessageHub.Abstractions` (namespace `Mpt.Framework.MessageHub`). Same members, separate namespaces — required because the project dependency direction is `Persistence → MessageHub` and never the reverse. Document the duplication in xmldoc on each copy. |
...
- **Same-simple-name interfaces across two abstractions packages** (MessageHub.Events + Persistence): `IEntityEventProducer<TEntity>` exists in *both* `Mpt.Framework.MessageHub` (authoring-layer `RegisterCustomEvent`/`ProduceCreatedEvents` shape) and `Mpt.Framework.Persistence` (Repository-driven `ProduceAsync` shape). Consumers importing both namespaces hit CS0104 on the bare type name; document this in the README and recommend full qualification rather than trying to merge the two interfaces — they serve different abstraction layers.
- **`PlatformEvent.IsTrusted` is `[Obsolete]` but still drives `EventHints.Incomplete`** (MessageHub.Events): even though `IsTrusted` is marked `[Obsolete("Use Hints instead")]` upstream, the `MakeMessage()` flow still reads it to OR `EventHints.Incomplete` into the message Hints. Keep the property AND the `if (!IsTrusted) msg.Hints |= EventHints.Incomplete;` line; drop only the upstream `msg.IsTrusted = ...` assignment (no OSS field). When `GenericDeletedEvent<T>` overrides `IsTrusted => false` the override must also carry `[Obsolete]` or you'll get CS0672 ("overrides obsolete member"); accept the obsolete-on-obsolete chain since the public API is unchanged.
...
- `src/messagehub/` — Abstractions + main, no EFCore. **Also the precedent for extending a previously-migrated package in place** (logical `Events/` subfolders, no new csprojs) when the new layer (a) depends on the engine package's transport infrastructure (MassTransit, `System.Threading.Channels`) and (b) is the natural completion of the package's stated purpose, not an orthogonal feature. The event-emission layer that was deliberately deferred in the Persistence migration eventually landed here: the full authoring API (`PlatformEvent` + `Generic{Created,Updated,Deleted,StatusChanged}Event<TEntity>` + `CustomEvent<TEntity>`), `IPlatformEventEmitter`, the upstream-style `IEntityEventProducer<TEntity>` with `RegisterCustomEvent` / `ProduceCustomEvents`, and an `EntityEventProducer<TEntity>` base class. Worth a glance for: (a) **two-tier publisher composition** — the existing `IMessageHubPublisher` stays the leaf MassTransit publisher; `IPlatformMessagePublisher` is added above it as a publish-mode router (Immediate delegates inline; Background pairs with a singleton `Channel<TracedTransport<EventMessage>>` + `IHostedService` drain). The two-tier shape lets us migrate both `Immediate` and `Background` publishers without touching the OSS-already-migrated leaf publisher. (b) **Accepting same-simple-name interface duplication across namespaces** when both are required to preserve upstream UX — `IEntityEventProducer<TEntity>` exists in both `Mpt.Framework.MessageHub` (upstream-style `ProduceCreatedEvents`/`RegisterCustomEvent` surface) and `Mpt.Framework.Persistence` (Repository-driven `ProduceAsync` surface). Document the clash in the README; don't try to merge. (c) **Enum duplication across two abstractions packages** (`EntityAction`, `EntityEventTypes` in both `Mpt.Framework.Persistence` and `Mpt.Framework.MessageHub`) when the dependency direction (`Persistence → MessageHub`, never reverse) prevents a shared definition. XmlDoc on each copy points at its sibling. (d) **Hollow but signature-preserving builders** — `PlatformEventPermissionsBuilder` retains its constructor-parameter slot on every `Generic*Event<TEntity>` (so upstream code ports verbatim) but its data does not flow to the wire because OSS `EventMessage` has no `Access`/`PrincipalAccess` fields. (e) **Flattening upstream types** when their sub-objects depend on business enums — `EventMessageActor` loses its `Account` sub-object (which carried a `UserAccountType` enum); the actor surface becomes flat `{ Id, Name?, Icon? }`. (f) **Forward-compat markers** — `ISyncPlatformEventProducer` + `IPlatformMessageReplayService` are shipped even though no caller wires them in this release, so consumers who need them later don't have to wait for another migration cycle.
...
- `src/persistence/` — Abstractions + main + EFCore. **The reference for combining multiple tightly-coupled upstream folders into a single OSS component.** Upstream's `Application/Query/`, `Application/Rules/`, and `Application/Data/` couldn't be shipped separately — the `Repository<T>` base class wires query reads, rules-driven lifecycle hooks, and event production into one save flow. The OSS port collapses all three into `Mpt.Framework.Persistence` with a unified `AddPersistence(moduleCode, configure)` builder. Key substitutions documented in this migration: (1) `ISecurityContextProvider` dropped entirely — the OSS Repository has no security overrides; consumers wrap their own scopes; (2) `ActorRoles` enum (Client / Vendor / Operations) → `IReadOnlyCollection<string>` role names, taxonomy is consumer-defined; (3) `IModuleConfiguration` → single `string moduleCode` on the builder; (4) `IPlatformEventEmitter` + the `PlatformEvent` authoring layer → `IMessageHubPublisher` from `Mpt.Framework.MessageHub.Abstractions`, with the producer returning `EventMessage` instances directly. Also the precedent for: (a) deferring a complex sub-feature (`EntityActionService` / Delta-applying rules engine) to a follow-up while shipping the rest; (b) optional dependency resolution (`sp.GetService<IMessageHubPublisher>()` returns null when MessageHub isn't registered, and the engine works fine without it); (c) `PersistenceBuilder.RepositoryTypeResolver` as the seam an EFCore add-on swaps in to redirect open-generic registrations.
